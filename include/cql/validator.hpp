...
public:
     ...
     */
    [[nodiscard]] ValidationSeverity severity() const {
        return m_severity;
    }
...
struct ValidationIssue {
     ...
     */
    ValidationIssue(const ValidationSeverity sev, std::string msg)
...
private:
    ...
    // Vector of validation rules - each rule is a function that takes a const vector of QueryNode unique_ptrs and
    // returns an optional ValidationIssue (if validation fails)
     ...
     */
    static std::map<TokenType, int> count_directives(const std::vector<std::unique_ptr<QueryNode>>& nodes);
