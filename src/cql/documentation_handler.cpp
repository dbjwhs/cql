...
int DocumentationHandler::handle_docs_command(int argc, char* argv[]) {
    if (argc < 3) {
        ...
        UserOutputManager::info("usage: cql --docs TEMPLATE_NAME");
        return CQL_ERROR;
    }
...
int DocumentationHandler::handle_export_command(int argc, char* argv[]) {
    if (argc < 3) {
        UserOutputManager::error("output path required");
        UserOutputManager::info("usage: cql --export OUTPUT_PATH [FORMAT]");
        return CQL_ERROR;
    }
    ...
    try {
        ...
        if (manager.export_documentation(output_path, format)) {
            UserOutputManager::success("template documentation exported to ", output_path,
                      " in ", format, " format");
        } else {
            UserOutputManager::error("failed to export template documentation");
            return CQL_ERROR;
        }
    } catch (const std::exception& e) {
